...
class GitHubDB:
    ...
    def put_file(self, path, data_dict):
        ...
        payload = {
            "message": f"update {path} via mention bot [skip ci] [skip render] [skip deploy] [render skip]",
            "content": content_b64,
            "branch": self.branch
        }
        if sha:
    ...
    try:
        for r in range(1, total_rounds + 1):
            ...
            for i in range(0, total_members, batch_size):
                ...
                is_last_batch = (r == total_rounds) and (i + batch_size >= total_members)
                if is_last_batch and not quiet:
                    import random
        ...
        logger.info("mention_all execution finished")
